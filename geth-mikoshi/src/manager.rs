...
use crate::utils::{chunk_filename_from, list_chunk_files, md5_hash_chunk_file};
...
impl ChunkManager {
        ...
        Events: Iterator<Item = ProposedEvent>,
    {
        ...
        while let Some(event) = events.next() {
            ...
            let prepare = PrepareLog {
                ...
                data: bytes::Bytes::from(event.data.to_vec()),
                metadata: bytes::Bytes::from(event.metadata.to_vec()),
            };
    ...
    pub fn last_epoch_record(&mut self) -> io::Result<Option<()>> {
        let last_epoch_pos = self.epoch_chk.read()?;

        // We have no evidence an epoch record was ever written. That doesn't mean
        // there is no epoch record, just that we have to look it for ourselves.
        if last_epoch_pos == -1 {}
