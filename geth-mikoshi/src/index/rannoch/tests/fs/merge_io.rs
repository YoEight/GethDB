...
use crate::index::rannoch::tests::{build_mem_table, check_merge_io_result, check_merge_result};
...
fn test_merge_io_mem_table_1() -> io::Result<()> {
    ...
    let mut merge_iter = MergeIO::new(iters);
...
fn test_merge_io_mem_table_2() -> io::Result<()> {
    ...
    let mut merge_iter = MergeIO::new(vec![
        mem_1.clone().into_iter().lift(),
        mem_2.clone().into_iter().lift(),
        mem_3.clone().into_iter().lift(),
